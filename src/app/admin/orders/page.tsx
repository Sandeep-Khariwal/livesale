...
//   const c = { PENDING: 0, VERIFIED: 0, REJECTED: 0, CANCELLED: 0, revenue: 0 };
...
//     if (o.paymentStatus === "CANCELLED") c.CANCELLED++;
//   }
//   return c;
// }, [orders]);
...
//         </button>
//                 <button
//           className={`stat-card ${filter === "CANCELLED" ? "stat-active" : ""}`}
//           onClick={() => setFilter(filter === "CANCELLED" ? "ALL" : "CANCELLED")}
//         >
//           <span className="stat-dot dot-muted" />
//           <div>
//             <p className="stat-num">{counts.CANCELLED}</p>
...
//     .dot-muted { background: var(--muted); box-shadow: 0 0 8px 1px rgba(168, 146, 123, 0.4); }
...
//     .status-cancelled {
//     background: rgba(168, 146, 123, 0.12);
//     color: var(--muted);
//   }
//   .status-cancelled .status-dot { background: var(--muted); }
...
//       .no-action-cancelled {
//     color: var(--muted);
//     background: rgba(168, 146, 123, 0.1);
//   }
...
export default function OrdersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter !== "ALL") {
      ...
      // Default view hides CANCELLED orders (customer backed out without
      // paying) so the main list stays clean — click the "Self-Cancelled
      // by Customer" stat card above to see them separately when needed.
    ...
    <div className={`${cormorant.variable} ${jost.variable} ord`}>
      ...
      <div className="stats">
        ...
        >
          ...
          <div>
            <p className="stat-num">{counts.CANCELLED}</p>
            <p className="stat-label">Self-Cancelled by Customer</p>
          </div>
        </button>
        <div className="stat-card stat-revenue">
          <div>
                ...
                return (
                  <tr key={order._id} className={order.paymentStatus === "PENDING" ? "row-pending" : ""}>
                    ...
                    <td>
                      <span className={`status-badge status-${order.paymentStatus?.toLowerCase()}`}>
                        <span className="status-dot" />
                        {order.paymentStatus === "CANCELLED" ? "CUSTOMER CANCELLED" : order.paymentStatus}
                      </span>
                    </td>
                    <td className="muted-cell">{order.orderStatus}</td>
